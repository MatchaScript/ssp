...
// Load static token data (extracted from @adobe/spectrum-tokens)
...
const semanticTokens: VariablesJson = JSON.parse(
	fs.readFileSync(path.join(tokensDir, 'semantic.json'), 'utf8')
);
const paletteTokens: PaletteJson = JSON.parse(
...
type PaletteJson = Record<string, PaletteToken>;
...
type VariablesJson = Record<string, TokenEntry>;
...
/**
 * Resolve a token reference string.
 * - Alias refs like `{accent-color-900}` → `var(--blue-900)` (compiled to palette direct ref)
 * - All other refs like `{gray-800}` → `var(--gray-800)` (kept as-is)
 */
	...
	return {
		...
		async load(id) {
			...
			function resolveTokenEntry(entry: TokenEntry): string | null {
				// Prefer sets with refs (light-dark resolution) over top-level ref
			...
			function genSemanticVars() {
				...
				for (const [key, entry] of Object.entries(semanticTokens)) {
					// Skip alias layer entries — they are compiled away
